...
jerror_t Paddles_test::evnt(JEventLoop *loop,uint64_t eventnumber)
{
	...
	for (data_it=data.begin();data_it<data.end();data_it++){			// loop paddles up - down
				...
				const fa250Mode1CalibHit *fa=faV[0];
						...
						if(evhit->m_channel.id==1){
							...
							for(int j=0;j<100;j++){
	                  		               ...
	                  		               amp_up_inv_del[j] = 0;
						                             	...
						                             	amp_up_inv_del[j] = -(fa->samples.at(j-2))+98;
							                            	} //end if-else
						...
						if(evhit->m_channel.id==0){
												for (int ii=0;ii<fa->samples.size();ii++){
													amp_down[ii]=(fa->samples.at(ii))-88;
                                                ...
                                                for(int j=0;j<100;j++){
                                                	...
                                                	amp_down_inv_del[j] = -(fa->samples.at(j-2))+88;        //segnale invertito e ritardato di 8ns
                                                	}
