...
void InitPlugin(JApplication *app) {
	InitJANAPlugin(app);
	app->AddProcessor(new Paddles_test());
}
} // "C"
...
//  (Constructor)
//------------------
Paddles_test::Paddles_test() :
		m_isFirstCallToBrun(1), m_ROOTOutput(0) {
...
// init
//------------------
jerror_t Paddles_test::init(void) {
	...
	t->Branch("amp_up", amp_up, "amp_up[100]/D");
	t->Branch("amp_down", amp_down, "amp_dpwn[100]/D");
	t->Branch("cfd_down", cfd_down, "cfd_down[100]/D");
	t->Branch("cfd_up", cfd_up, "cfd_up[100]/D");
	t->Branch("time", time, "time[100]/I");

	t->Branch("amp_down_att", amp_down_att, "amp_down_att[100]/D");
	t->Branch("amp_down_inv_del", amp_down_inv_del, "amp_down_inv_del[100]/D");
...
// brun
//------------------
jerror_t Paddles_test::brun(JEventLoop *loop, int32_t runnumber) {
...
// evnt
//------------------
jerror_t Paddles_test::evnt(JEventLoop *loop, uint64_t eventnumber) {
	...
	for (data_it = data.begin(); data_it < data.end(); data_it++) {			// loop paddles up - down
		...
		if (evhit->m_channel.id == 1) {
			...
			double amp_up_att[100];
			double amp_up_inv_del[100];
			for (int ii = 0; ii < fa->samples.size(); ii++) {
				amp_up[ii] = (fa->samples.at(ii) * 0.4884) - 98;
				amp_up_att[ii] = amp_up[ii] * 0.1;
			...
			for (int j = 0; j < 100; j++) {
				...
				if (j < 2) {
					...
					amp_up_inv_del[j] = -(fa->samples.at(j - 2) * 0.4884) + 98;
				} //end if-else
				cfd_up[j] = amp_up_att[j] + amp_up_inv_del[j];
			...
			for (int i = 0; i < 100; i++) {
				...
				if (i < 97) {
					...
					if (cfd_up[i + 1] < 0 && cfd_up[i] > 0) {
						//      jout << cfd_up[i+2] << " " << (i+2)*4 << " " <<cfd_up[i]<< " "<< i*4<< " "<< cfd_up[i+1]<< endl;

						Tup = ((((i + 1) - (i)) / (cfd_up[i + 1] - cfd_up[i])) * (0 - cfd_up[i])) + i;
		...
		if (evhit->m_channel.id == 0) {
			for (int ii = 0; ii < fa->samples.size(); ii++) {
				amp_down[ii] = (fa->samples.at(ii)* 0.4884) - 88;

				amp_down_att[ii] = amp_down[ii] * 0.1;              //segnale attenuato di 0.1
			...
			for (int j = 0; j < 100; j++) {
				...
				if (j < 2) {
					amp_down_inv_del[j] = 0;
				} else {
					amp_down_inv_del[j] = -(fa->samples.at(j - 2)* 0.4884) + 88;        //segnale invertito e ritardato di 8ns
				}
				cfd_down[j] = amp_down_att[j] + amp_down_inv_del[j];    // somma

				if (j <= (T_down / 4) || T_down == 0) {
					cfd_down[j] = 0;
				}
			...
			for (int i = 0; i < 100; i++) {
				...
				if (i < 97) {
					...
					if (cfd_down[i] > 0 && cfd_down[i + 1] < 0) {

						Td = ((((i + 1) - (i)) / (cfd_down[i + 1] - cfd_down[i])) * (0 - cfd_down[i])) + i;
...
// erun
//------------------
jerror_t Paddles_test::erun(void) {
...
// fini
//------------------
jerror_t Paddles_test::fini(void) {
