...
//	jout<<"Evt number="<< eventnumber<<" tWord= "<<tWord<<endl;
	...
	for (data_it = data.begin(); data_it < data.end(); data_it++) {
		...
		if (!fa)
			continue; // need fa250Mode1CalibHit to continue
...
//				jout<<"Component= "<<component<<endl;
		...
		for (int ii = 0; ii < fa->samples.size(); ii++) {
			...
			if (ii < 30)
				ped_first += fa->samples.at(ii) * 0.4884;
			if (ii > 80)
				ped_after += fa->samples.at(ii) * 0.4884;
...
//				jout<<"PED= "<<ped_first<<" "<<ped_after<<" "<<ped_evtbefore<<endl;
		...
		for (int ii = 0; ii < fa->samples.size(); ii++) {
			Q_first += (fa->samples.at(ii) * 0.4884 - ped_first);
			Q_after += (fa->samples.at(ii) * 0.4884 - ped_after);
			Q_evtbefore += (fa->samples.at(ii) * 0.4884 - ped_evtbefore);
...
//						jout<<thr_0_twc<<endl;
			for (int ii = 0; ii < fa->samples.size(); ii++) {
				if (fa->samples.at(ii) * 0.4884 > thr_0) {
					double Tinf = (ii - 1) * 4;
					double Tsup = ii * 4;
					T0 = Tinf + ((Tsup - Tinf) / 2);
					break;
			...
			for (int ii = 0; ii < fa->samples.size(); ii++) {
				if (fa->samples.at(ii) * 0.4884 > thr_0_twc) {
					double Tinf = (ii - 1) * 4;
					double Tsup = ii * 4;
...
//					jout<<"T0= "<<T0<<"T0_twc="<<T0_twc<<endl;
		...
		if (component == 1) {
			...
			for (int ii = 0; ii < fa->samples.size(); ii++) {
				if (fa->samples.at(ii) * 0.4884 > thr_1) {
					double Tinf = (ii - 1) * 4;
					double Tsup = ii * 4;
					T1 = Tinf + ((Tsup - Tinf) / 2);
					break;
